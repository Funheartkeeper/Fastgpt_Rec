...
/**
 * 记录跟踪视频点击量数据 trackResourceClick
 * @param data
 * @returns
 */
export const trackResourceClick = async (data: {
  resourceTitle: string;
  resourceUrl: string;
  dataId: string;
  eventType?: 'click' | 'dwell';
  dwellMs?: number;
  ...
  await POST('/core/chat/resource/click', data);
};

/**
 * 提交资源反馈信息 submitResourceFeedback
 * @param data
 * @returns
 */
export const submitResourceFeedback = async (data: {
  resourceTitle: string;
  resourceUrl: string;
  feedbackType: string;
  dataId: string;
}) => {
  // console.log('开始提交资源反馈，请求数据:', data);
  await POST('/core/chat/resource/feedback', data);
};
...
export const getRecommendedResources = (data: GetRecommendedResourcesParams) =>
  ...
  >('/core/chat/resource/getRecommendedResources', data);
