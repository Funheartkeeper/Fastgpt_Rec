  ...
  try {
    ...
    const handleToolCall = async (
      ...
      try {
        ...
        // 新增：尝试从请求上下文中获取更多变量（例如来自URL或请求头的studentUid）
        // 注意：这里的 req 对象需要包含你从分享链接解析出的变量。
        // 假设你的路由中间件已经将 query 中的变量放到了 req.body.customVariables 中
        const requestVariables = req.body.variables || {}; // 根据实际请求体字段调整
        ...
        const result = await callMcpServerTool({ 
          ...
          toolName: name, 
          inputs: args, 
          variables: requestVariables // 传递变量参数
        });
